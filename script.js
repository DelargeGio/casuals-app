...
function enviarMensaje() {
    ...
    // Sincronizado con usuario_nombre del login
...
// ======================================
// GESTIÓN DE PRESENCIA EN TIEMPO REAL (BLINDADA)
// ======================================
function iniciarPresencia() {
    if (typeof firebase === 'undefined') return;
    const usuario = localStorage.getItem("usuario_nombre");
    if (!usuario) {
        console.warn("⚠️ No hay usuario_nombre en localStorage para la presencia.");
        return;
    }
    ...
    firebase.database().ref('presencia').on('value', (snapshot) => {
        ...
        console.error("Error al leer la presencia de Firebase:", error);
        const contenedorNombres = document.getElementById("lista-nombres-conectados");
        if (contenedorNombres) contenedorNombres.innerHTML = "Error de conexión en presencia";
    });
}
...
// ======================================
// CONTROL DE VISTAS Y UTILIDADES GENERALES
// ======================================
...
window.cambiarVista = function(vista) {
    ...
    if (vista === "feed") {
        if (viewFeed) viewFeed.style.display = "flex";
